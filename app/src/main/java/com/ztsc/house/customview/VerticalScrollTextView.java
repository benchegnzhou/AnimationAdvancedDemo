...
import com.ztsc.house.bean.homeadvertisementbeen.ADEnity;
...
public class VerticalScrollTextView extends TextView {
    ...
    public double offsetTop;  //文字停留位置的偏移量
    ...
    public interface onClickLitener {
        ...
        public void onAdlick(String title, String mUrl);
    }
        ...
        switch (action) {
            case MotionEvent.ACTION_UP:
                if (onClickLitener != null) {
                    onClickLitener.onAdlick(mTexts.get(mIndex).getmBack(),mTexts.get(mIndex).getmUrl());
                }
    ...
    private void init() {
        ...
        mPaintFront.setTextSize(38);


        mPaintBack = new Paint();
        mPaintBack.setAntiAlias(true);
        mPaintBack.setDither(true);
        mPaintBack.setTextSize(42);
...
//                LogUtil.e(TAG, "onDraw: " + getMeasuredHeight());
            ...
            //移动到中间
            if (mY == getMeasuredHeight() / 2 - (indexBound.top + indexBound.bottom) / 2+ offsetTop) {
                isMove = false;
